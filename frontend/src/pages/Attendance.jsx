...
function MyAttendance() {
  ...
  function load() {
    api.get('/attendance/me', { params: { month, year } }).then(({ data }) => {
    ...
    <div className="page">
      ...
      <div className="welcome-banner">
        <div>
          ...
          <h3>{isCheckedIn ? "You're checked in" : "You're not checked in yet"}</h3>
        </div>
      ...
      <div className="section-header">
        ...
        <span className="muted">Count of days present: <strong>{presentCount}</strong></span>
      </div>

      <div className="card table-card">
        <table className="data-table">
          <thead>
            <tr><th>Date</th><th>Day</th><th>Check In</th><th>Check Out</th><th>Work Hours</th><th>Extra Hours</th><th>Status</th></tr>
          </thead>
          <tbody>
            {records.length === 0 && <tr><td colSpan={7} className="muted" style={{ textAlign: 'center' }}>No attendance records for this month.</td></tr>}
            {records.map((r) => (
              <tr key={r.id}>
...
function AllAttendance() {
  ...
  function load() {
    api.get('/attendance', { params: { date } }).then(({ data }) => setRecords(data.records)).catch((e) => setError(e.message));
  }
  ...
  return (
    <div className="page">
      ...
      <div className="stat-row">
        <div className="stat-card"><span className="stat-value">{records.length}</span><span className="stat-label">Total Employees</span></div>
        <div className="stat-card accent-green"><span className="stat-value">{presentCount}</span><span className="stat-label">Present</span></div>
        <div className="stat-card accent-amber"><span className="stat-value">{records.length - presentCount}</span><span className="stat-label">Absent / Leave</span></div>
      </div>
...
export default function Attendance() {
  const { user } = useAuth();
  return user?.role === 'admin' ? <AllAttendance /> : <MyAttendance />;
}
